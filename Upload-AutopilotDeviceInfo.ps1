...
param(
    ...
    [parameter(Mandatory=$false, HelpMessage="Specify the Client Secret, default this will be from DT&V ")]
    [ValidateNotNullOrEmpty()]
    [string]$clientsecret = "mbj7Q~NvxzKBdeyDS0i2fSqR_1G5aDHMs6k4R",
...
Begin {
    ...
    if ($Global:AuthToken -ne $null) {
        ...
        if ($TokenExpireMins -le 0) {
            ...
            $Global:AuthToken = Get-MSIntuneAuthToken -TenantName $TenantName -ClientID $ApplicationID -ClientSecret $clientsecret
        }
        else {
